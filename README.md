...
`AFSDKXPurchaseDetails` encapsulates all purchase necessary data for Android and iOS. 
...
* `purchaseToken` is the Android property, for Apple impl, please use empty string;
* The `transactionId` is used by Apple implementation, Android omits this property;

*Completion Handler Parameters:*

| Parameter | Type               | Description                                                      |
| --------- | ------------------ | ---------------------------------------------------------------- |
| response  | `cocos2d::ValueMap` | Contains the validation response data on success (empty on error) |
| error     | `cocos2d::ValueMap` | Contains error details on failure (empty on success)              |

*Error Map Properties:*
...
AppsFlyerX::validateAndLogInAppPurchase(details, params, [](cocos2d::ValueMap response, cocos2d::ValueMap error) { 
    if (!error.empty()) {
        // Handle error
        int errorCode = error["error_code"].asInt();
        std::string errorMessage = error["error_message"].asString();
        CCLOG("Validation failed - Code: %d, Message: %s", errorCode, errorMessage.c_str());
        return;
    }
    // Handle success - process response data
    CCLOG("Validation successful!");
});
```
